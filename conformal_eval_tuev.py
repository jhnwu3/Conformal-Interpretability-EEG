...
get_plots_research(test_loader, interpreter)
...
acc = validate_interpretable(test_loader, validator=validator, q_hat=0.0016499364282935858)
print("Accuracy of hard prediction sets using interpretability:", acc)
...
# cached_memory = torch.cuda.memory_cached(current_gpu)

# print("Allocated GPU memory (bytes):", allocated_memory)
# print("Cached GPU memory (bytes):", cached_memory)
...
#     pred_set = cal.predict_set(signal, q_hat)#generate_prediction_set(signal, model, q_hat, interpreter, e_score, class_index=label)
