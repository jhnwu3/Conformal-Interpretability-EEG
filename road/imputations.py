...
class NoisyLinearImputerTorch(BaseImputer):
    def __init__(self, noise=0.01, weighting=neighbors_weights):
        """	
            Noisy linear imputation.	
            noise: magnitude of noise to add (absolute, set to 0 for no noise)
    ...
    def setup_sparse_system(mask, img, neighbors_weights):
        ...
        indices = torch.nonzero(maskflt == 0).flatten() # Indices that are imputed in the flattened mask
        numEquations = len(indices)
        A = lil_matrix((numEquations, numEquations)) # System matrix
        b = torch.zeros((numEquations, img.shape[0]))
        sum_neighbors = torch.ones(numEquations) # Sum of weights assigned

        for n in neighbors_weights:
            offset, weight = n[0], n[1]
            valid, new_coords = NoisyLinearImputer.add_offset_to_indices(indices, offset, mask.shape)
            valid_coords = new_coords[valid]
            valid_ids = torch.nonzero(valid == 1).flatten()

            has_values_coords = valid_coords[maskflt[valid_coords] > 0.5]
            has_values_ids = torch.nonzero(maskflt[valid_coords] > 0.5).flatten()
            b[has_values_ids, :] -= weight * imgflat[:, has_values_coords].t()

            has_no_values = valid_coords[maskflt[valid_coords] < 0.5]
            variable_ids = torch.nonzero(maskflt[valid_coords] < 0.5).flatten()
            has_no_values_ids = torch.nonzero(valid == 1).flatten()
            A[has_no_values_ids, variable_ids] = weight

            sum_neighbors[torch.nonzero(valid == 0).flatten()] = sum_neighbors[torch.nonzero(valid == 0).flatten()] - weight

        A[torch.arange(numEquations), torch.arange(numEquations)] = -sum_neighbors  
        return A, b

    def __call__(self, img: torch.Tensor, mask: torch.Tensor):
        """ Our linear imputation scheme. """
        imgflt = img.view(img.shape[0], -1)
        maskflt = mask.view(-1)
        indices_linear = torch.nonzero(maskflt == 0).flatten() # Indices that need to be imputed.
        # Set up sparse equation system, solve system.
        A, b = NoisyLinearImputer.setup_sparse_system(mask, img, neighbors_weights)
        res = torch.tensor(spsolve(csc_matrix(A), b), dtype=torch.float)
    ...
    def batched_call(self, img: torch.Tensor, mask: torch.Tensor):
        ...
        in_device = img.device
        for i in range(len(img)):
            res_list.append(self.__call__(img[i].cuda(), mask[i].cuda()))
